...
// Set the address using the SoftDevice API.
    ...
    COMPAT_NRF_LOG_INFO("ble_set_mac_address: %02x:%02x:%02x:%02x:%02x:%02x", addr[5], addr[4], addr[3], addr[2], addr[1], addr[0]);
...
void _set_status(uint8_t status) { offline_finding_adv[6] = status; }
...
void set_battery(uint8_t battery_level) {
    ...
    COMPAT_NRF_LOG_INFO("Battery level: %d, status: %d%d", battery_level, (status_flag >> 7) & 1, (status_flag >> 6) & 1);
